...
if(perfLinearRegression):
    ...
    r_squared = gmp.getModelPredictions(model,X_train,Y_train,X_test,Y_test,outputPredictions,"Linear_Regression")
...
if(perfPolynomialRegression):
    ...
    r_squared = gmp.getModelPredictions(model,poly_X_train,Y_train,poly_X_test,Y_test,outputPredictions,"Polynomial_Regression")
    print("\nPolynomial Regression R_squared:", r_squared)

#ANN
if(perfANN):
    model = MLPRegressor(hidden_layer_sizes=(100,),solver="lbfgs",activation="relu")
    r_squared = gmp.getModelPredictions(model,X_train,Y_train,X_test,Y_test,outputPredictions,"ANN")
...
if(perfElasticNet):
    ...
    r_squared = gmp.getModelPredictions(model,X_train,Y_train,X_test,Y_test,outputPredictions,"Elastic_Net")
    print("\nElastic Net Regression R_squared:", r_squared)

#Random Forest
if(perfRandomForest):
    model = RandomForestRegressor()
    r_squared = gmp.getModelPredictions(model,X_train,Y_train,X_test,Y_test,outputPredictions,"Random_Forest")
